...
bool CBaseGame :: Update( void *fd, void *send_fd )
{
	...
	// rehost Rubattle
	if (m_GHost->m_RubattleBnetCount && !m_RefreshError && m_GameState==GAME_PUBLIC && GetTime()> m_LastRubattleRehostTime + (420/m_GHost->m_RubattleBnetCount) && !m_GameLoading && !m_GameLoaded && GetSlotsOpen()!=0)
	{
...
void CBaseGame :: SendWelcomeMessage( CGamePlayer *player )
{
	...
	if( in.fail( ) )
	{
		...
		SendChat( player, " " );
		SendChat( player, " " );
		SendChat( player, "|CFF00FF00_FIONA_	v. 0.0.1" );
		SendChat( player, "-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-" );
...
{	
	...
	// disable votestart, due to new user connected
