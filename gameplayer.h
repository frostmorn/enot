...
private:
	...
	std::vector<uint32_t> m_Pings;					// store the last few (3) pings received so we can take an average
...
public:
	...
	CGamePlayer( CPotentialPlayer *potential, unsigned char nPID, std::string nJoinedRealm, std::string nName, BYTEARRAY nInternalIP, bool nReserved, std::string nCountry );
	virtual ~CGamePlayer( );
