...
private:
	...
	#endif
	std::vector<PairedAdminCount> m_PairedAdminCounts;	// vector of paired threaded database admin counts in progress
	std::vector<PairedAdminAdd> m_PairedAdminAdds;		// vector of paired threaded database admin adds in progress
	std::vector<PairedAdminRemove> m_PairedAdminRemoves;	// vector of paired threaded database admin removes in progress
	std::vector<PairedBanCount> m_PairedBanCounts;		// vector of paired threaded database ban counts in progress
	...
	std::vector<CDBBan *> m_Bans;						// vector of cached bans
...
public:
	CBNET( CGHost *nGHost, std::string nServer, std::string nServerAlias, std::string nBNLSServer, uint16_t nBNLSPort, uint32_t nBNLSWardenCookie, std::string nCDKeyROC, std::string nCDKeyTFT, std::string nCountryAbbrev, std::string nCountry, uint32_t nLocaleID, std::string nUserName, std::string nUserPassword, std::string nFirstChannel, std::string nRootAdmin, char nCommandTrigger, bool nHoldFriends, 
		#ifdef GHOST_CLANS
	bool nHoldClan, 
	#endif
	bool nPublicCommands, unsigned char nWar3Version, BYTEARRAY nEXEVersion, BYTEARRAY nEXEVersionHash, std::string nPasswordHashType, std::string nPVPGNRealmName, uint32_t nMaxMessageLength, uint32_t nHostCounterID, uint32_t nLastGameCreateTime );
	~CBNET( );
