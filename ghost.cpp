...
CGHost :: CGHost( CConfig *CFG )
{
	...
	for( uint32_t i = 1; i < 256; ++i )
	{
		...
		HoldClan,
		#endif
		 PublicCommands, War3Version, EXEVersion, EXEVersionHash, PasswordHashType, PVPGNRealmName, MaxMessageLength, i, 0 ) );
	}
...
*/
		...
		if (((*i)->GetServerAlias().find("ICCup") == std::string::npos)&&((*i)->GetServerAlias().find("Rubattle") == std::string::npos))
		{
			...
			if( saveGame ){
				(*i)->QueueGameCreate( gameState, gameName + " " + UTIL_ToHexString(GetTime()), std::string( ), map, m_SaveGame, m_CurrentGame->GetHostCounter( ) );
			}
			else
			{
				(*i)->QueueGameCreate( gameState, gameName + " " +UTIL_ToHexString(GetTime()), std::string( ), map, NULL, m_CurrentGame->GetHostCounter( ) );
			}
		}
