			...
			******************/
			...
			else if( Command == "latency"  || Command == "l" )
			{
				...
				else
				{
					...
					{
						m_Latency = 5;
						SendAllChat( m_GHost->m_Language->SettingLatencyToMinimum( "5" ) );
					}
...
*/	
    ...
    else if( Command == "votestart" || Command == "go"|| Command == "го") 
    {
        ...
        if( m_GHost->m_VoteStartAllowed && !m_CountDownStarted && (votestartAuth || votestartAutohost || !m_GHost->m_VoteStartAutohostOnly) )
        {
            ...
            {
            //    string uname = m_NoName ? GetNoName( player->GetPID() ) : User;
                SendAllChat("[ " + player->GetName( ) + " ]  Проголосовал " + UTIL_ToString( Votes )+"/"+ UTIL_ToString(VotesNeeded)+ " необходимо для начала, регистрировать голос " + string( 1, m_GHost->m_CommandTrigger )+"го или " + string( 1, m_GHost->m_CommandTrigger )+ "go.");
            }
            else
            {
                StartCountDown( true );
