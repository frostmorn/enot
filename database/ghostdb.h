...
public:
	CCallableDotAPlayerAdd( uint32_t nGameID, uint32_t nColour, uint32_t nKills, uint32_t nDeaths, uint32_t nCreepKills,
	 uint32_t nCreepDenies, uint32_t nAssists, uint32_t nGold, uint32_t nNeutralKills, std::string nItem1, std::string nItem2, 
	 std::string nItem3, std::string nItem4, std::string nItem5, std::string nItem6, std::string nHero, uint32_t nNewColour, uint32_t nTowerKills,
	  uint32_t nRaxKills, uint32_t nCourierKills ) : CBaseCallable( ), m_GameID( nGameID ), m_Colour( nColour ), m_Kills( nKills ),
	   m_Deaths( nDeaths ), m_CreepKills( nCreepKills ), m_CreepDenies( nCreepDenies ), m_Assists( nAssists ), m_Gold( nGold ),
	    m_NeutralKills( nNeutralKills ), m_Item1( nItem1 ), m_Item2( nItem2 ), m_Item3( nItem3 ), m_Item4( nItem4 ), m_Item5( nItem5 ),
		 m_Item6( nItem6 ), m_Hero( nHero ), m_NewColour( nNewColour ), m_TowerKills( nTowerKills ), m_RaxKills( nRaxKills ), 
		 m_CourierKills( nCourierKills ), m_Result( 0 ) { }
...
protected:
	...
	uint32_t m_Result;
public:
	CCallableLiAPlayerAdd( uint32_t nGameID, uint32_t nColour, int32_t nPTS,
 uint32_t nDeaths, uint32_t nCreepKills, uint32_t nBossKills, std::string nItem1, std::string nItem2, std::string nItem3, std::string nItem4, std::string nItem5, std::string nItem6, std::string nHero ) : 
	CBaseCallable( ), m_GameID( nGameID ), m_Colour( nColour ), m_PTS( nPTS ), m_Deaths( nDeaths ), m_CreepKills( nCreepKills ), m_BossKills( nBossKills ),
	  m_Item1( nItem1 ), m_Item2( nItem2 ), m_Item3( nItem3 ), m_Item4( nItem4 ), m_Item5( nItem5 ), m_Item6( nItem6 ),
	   m_Hero( nHero ), m_Result( 0 ) { }
	virtual ~CCallableLiAPlayerAdd( );
...
public:
	CCallableLiAPlayerSummaryCheck( std::string nName ) : CBaseCallable( ), m_Name( nName ), m_Result( NULL ) { }
...
public:
	CDBLiAPlayer( );
	CDBLiAPlayer( uint32_t nID, uint32_t nGameID, uint32_t nColour, int32_t nPTS, uint32_t nDeaths, uint32_t nCreepKills, uint32_t nBossKills, std::string nItem1, std::string nItem2, std::string nItem3, std::string nItem4, std::string nItem5, std::string nItem6, std::string nHero);
	~CDBLiAPlayer( );

	uint32_t GetID( )				{ return m_ID; }
	uint32_t GetGameID( )			{ return m_GameID; }
	uint32_t GetColour( )			{ return m_Colour; }
	int32_t GetPTS()				{ return m_PTS; }
	uint32_t GetDeaths( )			{ return m_Deaths; }
	uint32_t GetCreepKills( )		{ return m_CreepKills; }
	uint32_t GetBossKills( )		{ return m_BossKills; }
	std::string GetItem( unsigned int i );
	std::string GetHero( )				{ return m_Hero; }

	void SetColour( uint32_t nColour )				{ m_Colour = nColour; }
	void SetPTS( int32_t nPTS )					{ m_PTS = nPTS; }
	void SetDeaths( uint32_t nDeaths )				{ m_Deaths = nDeaths; }
	void SetCreepKills( uint32_t nCreepKills )		{ m_CreepKills = nCreepKills; }
	void SetBossKills( uint32_t nBossKills )		{ m_BossKills = nBossKills; }
	void SetItem( unsigned int i, std::string item );
	void SetHero( std::string nHero )					{ m_Hero = nHero; }
};
...
private:
	...
	uint32_t m_TotalWins;			// total number of lia games won
	uint32_t m_TotalLosses;			// total number of lia games lost
	uint32_t m_TotalDeaths;			// total number of deaths
	int32_t m_TotalPTS;				// total number of PTS
	uint32_t m_TotalCreepKills;		// total number of creep kills
	uint32_t m_TotalBossKills;		// total number of boss kills
public:
	CDBLiAPlayerSummary( std::string nServer, std::string nName, uint32_t nTotalGames, uint32_t nTotalWins,
