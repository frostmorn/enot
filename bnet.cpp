...
bool CBNET :: Update( void *fd, void *send_fd )
{
	...
	if( m_Socket->GetConnecting( ) )
	{
		...
		if( m_Socket->CheckConnect( ) )
		{
			...
			m_Socket->PutBytes( m_Protocol->SEND_SID_AUTH_INFO( m_War3Version, m_GHost->m_TFT, m_LocaleID, m_CountryAbbrev, m_Country, m_Socket->LocalIP ) );
			m_Socket->DoSend( (fd_set *)send_fd );
			m_LastNullTime = GetTime( );
			m_LastOutPacketTicks = GetTicks( );
...
void CBNET :: BotCommand( string Message, string User, bool Whisper, bool ForceRoot ) {
	...
	*********************/
	...
	if( IsAdmin( User ) || IsRootAdmin( User ) || ForceRoot || ( m_PublicCommands && m_OutPackets.size( ) <= 3 ) )
	{
		...
		{
			if( Payload.empty( ) )
				QueueChatCommand( "stats votekick calc bug", User, Whisper );
			else
			{
