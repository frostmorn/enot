...
bool CBNLSClient :: Update( void *fd, void *send_fd )
{
	...
	{
		CONSOLE_Print( "[BNLSC: " + m_Server + ":" + std::to_string( m_Port ) + ":C" + std::to_string( m_WardenCookie ) + "] disconnected from BNLS server due to socket error" );
		return true;
	}

	if( !m_Socket->GetConnecting( ) && !m_Socket->GetConnected( ) && m_WasConnected )
	{
		CONSOLE_Print( "[BNLSC: " + m_Server + ":" + std::to_string( m_Port ) + ":C" + std::to_string( m_WardenCookie ) + "] disconnected from BNLS server" );
		return true;
	}
	...
	{
		CONSOLE_Print( "[BNLSC: " + m_Server + ":" + std::to_string( m_Port ) + ":C" + std::to_string( m_WardenCookie ) + "] connected" );
		m_WasConnected = true;
		m_LastNullTime = GetTime( );
		return false;
	}
	...
	if( !m_Socket->GetConnecting( ) && !m_Socket->GetConnected( ) && !m_WasConnected )
	{
		CONSOLE_Print( "[BNLSC: " + m_Server + ":" + std::to_string( m_Port ) + ":C" + std::to_string( m_WardenCookie ) + "] connecting to server [" + m_Server + "] on port " + std::to_string( m_Port ) );
...
void CBNLSClient :: ExtractPackets( )
{
	...
	while( Bytes.size( ) >= 3 )
	{
		...
		{
			CONSOLE_Print( "[BNLSC: " + m_Server + ":" + std::to_string( m_Port ) + ":C" + std::to_string( m_WardenCookie ) + "] error - received invalid packet from BNLS server (bad length), disconnecting" );
			m_Socket->Disconnect( );
			return;
		}
	}
